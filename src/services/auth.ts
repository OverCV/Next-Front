...
export const authService = {
    ...
    estaAutenticado: (): boolean => {
        ...
        console.log('🔑 Verificando autenticación:', { token: token ? 'exists' : 'null' })
        return !!token
    },
        ...
        if (!token) {
            ...
            if (usuarioStr) {
                try {
                    ...
                    console.error('Error al parsear el usuario para obtener token:', error)
                }
            }
        }

        console.log('🎫 Obteniendo token:', { token: token ? `${token.substring(0, 15)}...` : 'null' })
        return token || null
    },
